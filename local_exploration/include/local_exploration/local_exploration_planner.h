#ifndef LOCAL_EXPLORATION_PLANNER_H_
#define LOCAL_EXPLORATION_PLANNER_H_
...
#include "global_repositioning/repositioning_region.h"
#include "subt_planner_visualization/mission_visuals.h"
#include "cognitive_graph_model/geofence_manager.h"
#include "cognitive_graph_model/graph_storage.h"
#include "cognitive_graph_model/graph_types.h"
#include "cognitive_graph_model/cognitive_graph.h"
#include "cognitive_graph_model/params.h"
#include "cognitive_graph_model/random_sampler.h"
#include "cognitive_graph_model/trajectory.h"
#include "cognitive_graph_model/utils.h"
...
// Enable full RViz topic publishing.
#define FULL_PLANNER_VIZ 1

static const double max_difference_waypoint_to_graph = 15.0;
static constexpr double kSmallNorm = 1e-6;


// Track robot states for topology updates.
...
class LocalExplorationPlanner {
 public:
  ...
  LocalExplorationPlanner(const ros::NodeHandle& nh, const ros::NodeHandle& nh_private);
  // Reuse a shared voxel map when the manager owns map construction.
  LocalExplorationPlanner(const ros::NodeHandle& nh, const ros::NodeHandle& nh_private,
      VoxelMap* voxel_map);
  ...
  void expandGraphEdgesBatch(std::shared_ptr<CognitiveGraph> graph_manager,
                        std::vector<Vertex> sampled_vertices);
  void expandGraph(std::shared_ptr<CognitiveGraph> graph_manager,
  ...
  void expandGraphEdges(std::shared_ptr<CognitiveGraph> graph_manager,
                        Vertex* new_vertex, ExpandGraphReport& rep);

  void expandGraphEdgesBlindly(std::shared_ptr<CognitiveGraph> graph_manager,
  ...
  ConnectStatus findPathToConnect(StateVec& source, StateVec& target,
                                  std::shared_ptr<CognitiveGraph> graph_manager,
  ...
  bool lastGlobalPathFailedNoFrontier() const {
    return last_global_path_failed_no_frontier_;
  }
  ...
  std::vector<geometry_msgs::Pose> connectTSPOrder(std::vector<int> &tsp_nodes, std::map<int, ShortestPathsReport> &path_rep_map, std::shared_ptr<CognitiveGraph> graph);
  std::vector<geometry_msgs::Pose> connectTSPOrderWithSubvertices(std::vector<std::pair<int, std::vector<int>>> &tsp_nodes, std::map<int, ShortestPathsReport> &path_rep_map, std::shared_ptr<CognitiveGraph> graph);
 ...
 private:
  bool sampleVertexCandidate(RandomSampler& random_sampler,
                             const StateVec& root_state, Vertex& vertex,
                             bool apply_geofence_check,
                             bool apply_axis_bias);
  void applyAxisBiasedSampling(const Eigen::Vector3d& reference_pos,
                               StateVec& state) const;
  void updatePassableDirectionField(const Eigen::Vector3d& reference_pos);
  bool samplePassableDirection(const StateVec& root_state, StateVec& state);
  ...
  double getClearanceScore(const Eigen::Vector3d& position) const;
  Eigen::Vector3d getPrimaryExplorationAxis(
      const Eigen::Vector3d& reference_pos) const;
  bool isZonePartitionActive() const;
  int getZoneIndex(const Eigen::Vector3d& position) const;
  double getZoneTravelFactor(const Eigen::Vector3d& source,
                             const Eigen::Vector3d& target) const;
  double computeSampleUtility(const StateVec& state,
                              const Eigen::Vector3d& reference_pos) const;
  void publishAblationMetrics();
  void accumulateSamplingAttempt(bool valid_sample);
  void accumulateViewpointEvaluation(int candidate_count,
                                     int evaluated_count);
  void accumulateGlobalFallbackPathLength(
      const std::vector<geometry_msgs::Pose>& path);
  void accumulateGlobalFrontierFallbackMetrics(
      const std::vector<geometry_msgs::Pose>& path);
  void shortlistVerticesForFineGain(bool only_leaf_vertices,
                                    std::unordered_set<int>& selected_ids);
  void augmentFreeStartupArea(const StateVec& state, double reach);
  double getLocalNavigationRecoveryReach() const;
  bool maybeRecoverLocalNavigation(const Eigen::Vector3d& local_target,
                                   const std::string& reason);
  // Correct the heading of each vertex in the Dijkstra shortest paths in the
  // exploration graph to follow the tangent of each segment.
  void correctYaw();
  ...
  // Submit local frontiers to the cognitive map.
  void addFrontiers(int best_vertex_id);
  int addOpeningFrontierSeeds();
  ...
  ros::Publisher ablation_log_pub_;
  ...
  // Graphs.
  std::shared_ptr<CognitiveGraph> local_graph_;
  ShortestPathsReport local_graph_rep_;  // shortest path to root vertex
  std::shared_ptr<CognitiveGraph> global_graph_;
  ...
  int local_navigation_recovery_attempts_ = 0;

  // Commit verified exploration evidence to the cognitive map.
  bool commitVerifiedPathToCognitiveMap(const std::shared_ptr<CognitiveGraph> graph_manager,
                         const std::vector<Vertex*>& vertices);
  bool commitVerifiedPathToCognitiveMap(const std::shared_ptr<CognitiveGraph> graph_manager,
                         const std::vector<geometry_msgs::Pose>& path);
  bool anchorStateInCognitiveMap(std::shared_ptr<CognitiveGraph> graph,
                           StateVec& cur_state, Vertex*& v_added,
                           double dist_ignore_collision_check);
  struct BranchFrontierCandidate {
    Vertex* frontier = NULL;
    double current_to_frontier_distance = 0.0;
    double frontier_to_home_distance = 0.0;
    double time_to_target = 0.0;
    double time_to_home = 0.0;
    double time_cost = 0.0;
    double time_spare = 1.0;
    double exp_gain = 0.0;
  };
  struct BranchRegionCandidate {
    int entry_vertex_id = -1;
    std::vector<const BranchFrontierCandidate*> frontiers;
    double aggregated_gain = 0.0;
    double current_to_entry_distance = 0.0;
    double entry_to_home_distance = 0.0;
    double score = 0.0;
  };
  enum class BranchTargetSelectionMode {
    kPoint,
    kRegion,
  };
  bool selectGlobalFrontierTarget(
  ...
  int findBranchRegionEntry(
      Vertex* frontier, const ShortestPathsReport& home_graph_rep) const;
  void updateAdaptiveGlobalSelectionTimings(double point_time,
                                            double region_time);
  bool shouldUseBranchRegionSelection(
      double point_time, double region_time, double point_gain,
      double region_gain) const;
  BranchTargetSelectionMode chooseBranchTargetSelectionMode(
      const std::vector<Vertex*>& global_frontiers,
      const ShortestPathsReport& frontier_graph_rep) const;
  double estimateBranchRegionCompactness(
      const std::vector<Vertex*>& global_frontiers,
      const ShortestPathsReport& home_graph_rep) const;
  void updateAdaptiveGlobalFrontierCountStats(int frontier_count);
  void updateAdaptiveBranchRegionSuccess(bool success);
  void applyGlobalTargetHysteresis(
      const std::vector<BranchFrontierCandidate>& candidates,
      Vertex*& best_frontier, double& best_gain) const;
  bool isDeadZoneCandidate(bool frontier_exists, double best_gain,
                           int num_leaf_vertices) const;
  int getTopologicalMapDegree(const Vertex* vertex) const;
  void markDeadZoneRegion(Vertex* center_vertex);
  double getDeadZoneDistance(const Eigen::Vector3d& position) const;
  double getDeadZonePenalty(const Eigen::Vector3d& position) const;
  bool tryPrependDeadZoneEscapeManeuver(
      std::vector<geometry_msgs::Pose>& path) const;
  bool buildUShapedEscapeManeuver(
      double lateral_sign, const std::vector<geometry_msgs::Pose>& follow_path,
      std::vector<geometry_msgs::Pose>& escape_path) const;
  void maybeAddShortcutEdges(const std::shared_ptr<CognitiveGraph> graph_manager,
                             Vertex* new_vertex);
  double estimatePathClearance(const Eigen::Vector3d& start,
  ...
  const double kTopologicalMapUpdateTimerPeriod = 0.5;
  const double kTopologicalFrontierAdditionTimerPeriod = 1.0;
  const double kTopologicalMapUpdateTimeBudget = 0.1;
  ros::Timer topological_map_update_timer_;
  void expandTopologicalMapTimerCallback(const ros::TimerEvent& event);
  ros::Timer topological_frontier_addition_timer_;
  void expandTopologicalFrontierAdditionTimerCallback(
  ...
  inline void offsetZAxis(StateVec&, bool down = false) {}
  ...
  RepositioningRegion* repositioning_region_;
  ...
  bool dead_zone_escape_pending_ = false;
  int latest_dead_zone_vertex_id_ = -1;
  std::vector<int> dead_zone_vertex_ids_;
  double branch_repositioning_point_time_ema_ = -1.0;
  double branch_repositioning_region_time_ema_ = -1.0;
  double branch_repositioning_frontier_count_ema_ = -1.0;
  double branch_repositioning_frontier_count_sq_ema_ = -1.0;
  double branch_repositioning_success_ema_ = 0.5;
  bool branch_repositioning_cache_ready_ = false;
  bool last_global_path_failed_no_frontier_ = false;

  bool global_exploration_ongoing_;
  int current_global_vertex_id_;
  bool local_exploration_ongoing_;
  bool passable_direction_field_valid_ = false;
  Eigen::Vector3d passable_direction_field_reference_ = Eigen::Vector3d::Zero();
  std::vector<std::pair<double, Eigen::Vector3d>> passable_direction_candidates_;
  bool last_gain_eval_partial_ = false;
  bool force_full_gain_evaluation_ = false;
  std::uint64_t total_sampling_attempts_ = 0;
  std::uint64_t total_valid_samples_ = 0;
  std::uint64_t total_candidate_viewpoints_ = 0;
  std::uint64_t total_evaluated_viewpoints_ = 0;
  double total_global_fallback_path_length_ = 0.0;
  double total_global_frontier_fallback_path_length_ = 0.0;
  double total_global_frontier_fallback_time_ = 0.0;
  std::uint64_t total_global_frontier_fallback_count_ = 0;
