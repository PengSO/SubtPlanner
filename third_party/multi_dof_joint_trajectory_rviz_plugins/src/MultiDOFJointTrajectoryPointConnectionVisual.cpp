...
: scene_manager_(scene_manager),
  ...
  color_(color)
{
  const std::size_t count = std::min(from.transforms.size(), to.transforms.size());
  if (from.transforms.size() != to.transforms.size()) {
    ROS_WARN_STREAM("MultiDOFJointTrajectoryPointConnectionVisual: transform vectors differ in length. from="
                    << from.transforms.size() << " to=" << to.transforms.size());
  }
