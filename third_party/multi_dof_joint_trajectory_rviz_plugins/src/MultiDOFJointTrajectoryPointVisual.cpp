...
namespace multi_dof_joint_trajectory_rviz_plugins
{
        ...
        show_text_(show_text)
  {
    ...
    if (!captions.empty() && captions.size() != msg.transforms.size()) {
      ROS_WARN_STREAM("MultiDOFJointTrajectoryPointVisual: captions and transforms vectors differ in length. captions="
                      << captions.size() << " transforms=" << msg.transforms.size());
    }
  ...
  {
    const std::size_t count = std::min(texts_.size(), captions_.size());
