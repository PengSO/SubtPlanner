...
namespace explorer
{
      ...
      : PCIManager(nh, nh_private)
  {
    ...
    is_homing_pub_ = nh_.advertise<std_msgs::Bool>("/subt_planner_is_homing", 10);
  ...
  void PCIGeneral::statusCallback(const mav_msgs::Status &status)
  {
    ...
    // ARL-M100 voltage-to-remaining-time calibration.
  ...
  bool PCIGeneral::initMotion()
  {
    ...
    progress_waypoint_ind_ = 0;
    best_waypoint_distance_ = std::numeric_limits<double>::infinity();
    best_waypoint_yaw_error_ = std::numeric_limits<double>::infinity();
    best_waypoint_pitch_error_ = std::numeric_limits<double>::infinity();
    last_path_progress_time_ = ros::Time::now();
  ...
  void PCIGeneral::executionTimerCallback(const ros::TimerEvent &event)
  {
    ...
    if (pci_status_ == PCIStatus::kRunning)
    {
      ...
      if (path_waypoint_ind_ != progress_waypoint_ind_)
      {
        progress_waypoint_ind_ = path_waypoint_ind_;
        best_waypoint_distance_ = std::numeric_limits<double>::infinity();
        best_waypoint_yaw_error_ = std::numeric_limits<double>::infinity();
        best_waypoint_pitch_error_ = std::numeric_limits<double>::infinity();
        last_path_progress_time_ = now;
      }

      const double dist_progress_min =
          std::max(0.05, 0.25 * path_progression_dist_thr_min_);
      const double yaw_progress_min =
          std::max(0.05, 0.25 * path_progression_yaw_thr_min_);
      const double pitch_progress_min =
          std::max(0.02, 0.25 * path_progression_pitch_thr_);
      const bool progress_in_distance =
          std::isfinite(best_waypoint_distance_) &&
          (best_waypoint_distance_ - wp_dist) > dist_progress_min;
      const bool progress_in_yaw =
          std::isfinite(best_waypoint_yaw_error_) &&
          (best_waypoint_yaw_error_ - delta_yaw) > yaw_progress_min;
      const bool progress_in_pitch =
          std::isfinite(best_waypoint_pitch_error_) &&
          (best_waypoint_pitch_error_ - delta_pitch) > pitch_progress_min;
      if (progress_in_distance || progress_in_yaw || progress_in_pitch)
      {
        last_path_progress_time_ = now;
      }
      best_waypoint_distance_ = std::min(best_waypoint_distance_, wp_dist);
      best_waypoint_yaw_error_ =
          std::min(best_waypoint_yaw_error_, delta_yaw);
      best_waypoint_pitch_error_ =
          std::min(best_waypoint_pitch_error_, delta_pitch);
      ...
      else if (path_progress_timeout_ > 0.0 &&
               last_path_progress_time_.isValid() &&
               (now - last_path_progress_time_).toSec() >
                   path_progress_timeout_)
      {
        ROS_WARN_COND(
            global_verbosity >= Verbosity::WARN,
            "PCI: no waypoint progress for %.2f s (wp=%d, dist=%.2f, yaw=%.2f, pitch=%.2f).",
            (now - last_path_progress_time_).toSec(), path_waypoint_ind_,
            wp_dist, delta_yaw, delta_pitch);
        pci_status_ = PCIStatus::kError;
      }
      if (pci_status_ == PCIStatus::kRunning)
      {
  ...
  bool PCIGeneral::loadParams(const std::string ns)
  {
    ...
    param_name = ns + "/path_progress_timeout";
    if (!ros::param::get(param_name, path_progress_timeout_))
    {
      path_progress_timeout_ = 12.0;
      ROS_WARN_COND(param_verbosity >= Verbosity::WARN,
                    "No path_progress_timeout setting, setting it to %f (s).",
                    path_progress_timeout_);
    }
