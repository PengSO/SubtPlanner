...
PlannerControlInterface::PlannerControlInterface(
  ...
  planner_status_pub_ = nh_.advertise<std_msgs::Bool>("subt_planner_status", 5);
  stop_request_pub_ = nh_.advertise<std_msgs::Bool>(
      "planner_control_interface/stop_request", 5);
  global_revisit_log_pub_ = nh_.advertise<std_msgs::Float64MultiArray>(
      "planner_control_interface/global_revisit_log", 10);
  ...
  planner_search_client_ =
      nh.serviceClient<planner_msgs::planner_search>("subt_planner/search");
  ...
  planner_global_client_ =
      nh.serviceClient<planner_msgs::planner_global>("subt_planner/global");
  ...
  planner_geofence_client_ =
      nh.serviceClient<planner_msgs::planner_geofence>("subt_planner/geofence");
  ...
  planner_inspection_srv_client_ =
      nh.serviceClient<planner_msgs::planner_srv>("/subt_planner/get_inspection_path");
  ...
  nav_goal_client_ = nh_.serviceClient<planner_msgs::planner_go_to_waypoint>(
      "subt_planner/go_to_waypoint");
  go_to_waypoint_visualization_pub_ = nh_.advertise<visualization_msgs::Marker>(
      "subt_planner/go_to_waypoint_pose_visualization", 0);
...
void PlannerControlInterface::setGoal(const geometry_msgs::PoseStamped& pose) {
  ...
  try {
    ...
    ROS_ERROR_COND(global_verbosity >= Verbosity::ERROR,
                   "[subt_planner_pci::setGoal] %s", ex.what());
    return;
  }
  ...
  ROS_INFO_COND(
      ...
      "[subt_planner_pci::setGoal] set waypoint to (%.2f, %.2f, %.2f) in frame "
      "'%s'",
...
void PlannerControlInterface::resetPlanner() {
  if (global_revisit_active_) {
    endGlobalRevisitTiming(ros::Time::now());
  }
...
bool PlannerControlInterface::init() {
  ...
  go_to_waypoint_request_ = false;
  global_revisit_active_ = false;
  global_revisit_start_time_ = ros::Time();
  last_global_revisit_publish_time_ = ros::Time();
  total_global_revisit_time_s_ = 0.0;
  total_global_revisit_distance_m_ = 0.0;
  active_global_revisit_distance_m_ = 0.0;
  last_global_revisit_distance_m_ = 0.0;
  total_global_revisit_count_ = 0;
  last_global_revisit_pose_valid_ = false;
  previous_pci_status_ = PCIManager::PCIStatus::kReady;
...
void PlannerControlInterface::run() {
  ...
  while (cont) {
    ...
    if (global_revisit_active_ &&
        previous_pci_status_ == PCIManager::PCIStatus::kRunning &&
        pci_status != PCIManager::PCIStatus::kRunning) {
      endGlobalRevisitTiming(now);
    }
    previous_pci_status_ = pci_status;
    if (!last_global_revisit_publish_time_.isValid() ||
        (now - last_global_revisit_publish_time_).toSec() >= 1.0) {
      publishGlobalRevisitLog();
    }
    if (pci_status == PCIManager::PCIStatus::kReady) {
      ...
      const bool had_active_path = !current_path_.empty();
      resetPlanner();
      if (had_active_path) {
        run_en_ = true;
        ROS_WARN_COND(global_verbosity >= Verbosity::WARN,
                      "PCI: execution stalled, auto-triggering replanning.");
      }
    }
    cont = ros::ok();
    ros::spinOnce();
      ...
      if (!plan_srv.response.path.empty()) {
        ...
        if (exe_path) {
          if ((!force_forward_) ||
            ...
            if (plan_srv.response.is_global_fallback_path) {
              path_type = PCIManager::ExecutionPathType::kGlobalPath;
            }
            if (path_type == PCIManager::ExecutionPathType::kGlobalPath) {
              beginGlobalRevisitTiming();
            }
            pci_manager_->executePath(plan_srv.response.path, path_to_be_exe,
                                      path_type);
        ...
        const bool phase_transition_complete =
            plan_srv.response.status ==
            planner_msgs::planner_srv::Response::kAutoCustomPath;
        publishPlannerStatus(plan_srv.response, phase_transition_complete);
        if (phase_transition_complete) {
          current_path_.clear();
          run_en_ = true;
          ROS_WARN_COND(
              global_verbosity >= Verbosity::PLANNER_STATUS,
              "PCI: local navigation phase completed, continuing planning.");
          success = true;
        } else {
          ROS_WARN_COND(global_verbosity >= Verbosity::WARN,
        ...
        if (plan_srv.response.status == plan_srv.response.kHoming ||
          ...
          if (had_active_path &&
            ...
            run_en_ = true;
            ROS_WARN_COND(global_verbosity >= Verbosity::WARN,
                          "PCI: planner returned a stuck/manual-recovery path, auto-triggering replanning.");
          }
          success = true;
        }
        // ros::Duration(0.5).sleep();
...
void PlannerControlInterface::beginGlobalRevisitTiming() {
  const ros::Time now = ros::Time::now();
  if (global_revisit_active_) {
    endGlobalRevisitTiming(now);
  }
  global_revisit_active_ = true;
  global_revisit_start_time_ = now;
  active_global_revisit_distance_m_ = 0.0;
  last_global_revisit_distance_m_ = 0.0;
  last_global_revisit_pose_ = current_pose_.position;
  last_global_revisit_pose_valid_ = pose_is_ready_;
  ++total_global_revisit_count_;
  publishGlobalRevisitLog();
}

void PlannerControlInterface::endGlobalRevisitTiming(const ros::Time& stamp) {
  if (!global_revisit_active_ || !global_revisit_start_time_.isValid()) {
    return;
  }
  const double duration_s =
      std::max(0.0, (stamp - global_revisit_start_time_).toSec());
  total_global_revisit_time_s_ += duration_s;
  total_global_revisit_distance_m_ += active_global_revisit_distance_m_;
  last_global_revisit_distance_m_ = active_global_revisit_distance_m_;
  global_revisit_active_ = false;
  global_revisit_start_time_ = ros::Time();
  active_global_revisit_distance_m_ = 0.0;
  last_global_revisit_pose_valid_ = false;
  publishGlobalRevisitLog(duration_s);
}

void PlannerControlInterface::updateGlobalRevisitDistance() {
  if (!global_revisit_active_) {
    return;
  }
  if (!last_global_revisit_pose_valid_) {
    last_global_revisit_pose_ = current_pose_.position;
    last_global_revisit_pose_valid_ = true;
    return;
  }

  const Eigen::Vector3d prev(last_global_revisit_pose_.x,
                             last_global_revisit_pose_.y,
                             last_global_revisit_pose_.z);
  const Eigen::Vector3d curr(current_pose_.position.x, current_pose_.position.y,
                             current_pose_.position.z);
  const double segment_m = (curr - prev).norm();
  const double kMaxPoseJumpForDistanceM = 5.0;
  if (segment_m <= kMaxPoseJumpForDistanceM) {
    active_global_revisit_distance_m_ += segment_m;
  } else {
    ROS_WARN_COND(global_verbosity >= Verbosity::WARN,
                  "PCI: skipped %.3fm pose jump while measuring global revisit distance.",
                  segment_m);
  }
  last_global_revisit_pose_ = current_pose_.position;
}

void PlannerControlInterface::publishGlobalRevisitLog(double last_duration_s) {
  const ros::Time now = ros::Time::now();
  double active_duration_s = 0.0;
  if (global_revisit_active_ && global_revisit_start_time_.isValid()) {
    active_duration_s = std::max(0.0, (now - global_revisit_start_time_).toSec());
  }
  std_msgs::Float64MultiArray log;
  log.data.reserve(7);
  log.data.push_back(total_global_revisit_time_s_ + active_duration_s);
  log.data.push_back(static_cast<double>(total_global_revisit_count_));
  log.data.push_back(global_revisit_active_ ? 1.0 : 0.0);
  log.data.push_back(last_duration_s);
  log.data.push_back(total_global_revisit_distance_m_ +
                     active_global_revisit_distance_m_);
  log.data.push_back(last_global_revisit_distance_m_);
  log.data.push_back(active_global_revisit_distance_m_);
  global_revisit_log_pub_.publish(log);
  last_global_revisit_publish_time_ = now;
}
