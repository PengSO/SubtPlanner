...
#include "voxel_map/voxblox_map_backend.h"

// TSDF
template <typename SDFServerType, typename SDFVoxelType>
voxblox::Layer<SDFVoxelType>*
VoxbloxMapBackend<SDFServerType, SDFVoxelType>::getSDFLayer() {
...
VoxbloxMapBackend<voxblox::EsdfServer, voxblox::EsdfVoxel>::getSDFLayer() {
  return sdf_server_.getEsdfMapPtr()->getEsdfLayerPtr();
}

template <typename SDFServerType, typename SDFVoxelType>
VoxbloxMapBackend<SDFServerType, SDFVoxelType>::VoxbloxMapBackend(
  ...
  if (!nh_private.getParam("occupancy_distance_voxelsize_factor",
    ...
    ROS_INFO_COND(global_verbosity >= Verbosity::INFO,
                  "VoxbloxMapBackend: failed to find parameter for "
  ...
  ROS_INFO_COND(global_verbosity >= Verbosity::INFO,
                "[VoxelMap]: Point collision checking method: Box check");
#elif (COL_CHECK_METHOD == 1)
  ROS_INFO_COND(global_verbosity >= Verbosity::INFO,
                "[VoxelMap]: Point collision checking method: Direct T/ESDF");
#elif (COL_CHECK_METHOD == 2)
  ROS_INFO_COND(
      global_verbosity >= Verbosity::INFO,
      "[VoxelMap]: Point collision checking method: Interpolated T/ESDF");
#endif

#if (EDGE_CHECK_METHOD == 0)
  ROS_INFO_COND(
      global_verbosity >= Verbosity::INFO,
      "[VoxelMap]: Line collision checking method: Multiple box checks");
#elif (EDGE_CHECK_METHOD == 1)
  ROS_INFO_COND(
      global_verbosity >= Verbosity::INFO,
      "[VoxelMap]: Line collision checking method: Cuboid around the");
#elif (EDGE_CHECK_METHOD == 2)
  ROS_INFO_COND(
      global_verbosity >= Verbosity::INFO,
      "[VoxelMap]: Line collision checking method: Direct T/ESDF check");
#elif (EDGE_CHECK_METHOD == 3)
  ROS_INFO_COND(
      global_verbosity >= Verbosity::INFO,
      "[VoxelMap]: Line collision checking method: Interpolated T/ESDF "
      "check");
#endif

#if (RAY_CAST_METHOD == 0)
  ROS_INFO_COND(global_verbosity >= Verbosity::INFO,
                "[VoxelMap]: Ray casting method: Original");
#elif (RAY_CAST_METHOD == 1)
  ROS_INFO_COND(global_verbosity >= Verbosity::INFO,
                "[VoxelMap]: Ray casting method: Iterative");
#endif
}

template <typename SDFServerType, typename SDFVoxelType>
double VoxbloxMapBackend<SDFServerType, SDFVoxelType>::getResolution() {
  return sdf_layer_->voxel_size();
}

template <typename SDFServerType, typename SDFVoxelType>
bool VoxbloxMapBackend<SDFServerType, SDFVoxelType>::getStatus() {
  return true;
}

// TSDF
template <typename SDFServerType, typename SDFVoxelType>
bool VoxbloxMapBackend<SDFServerType, SDFVoxelType>::checkUnknownStatus(
...
// ESDF
template <>
bool VoxbloxMapBackend<voxblox::EsdfServer, voxblox::EsdfVoxel>::
...
template <typename SDFServerType, typename SDFVoxelType>
VoxelStatus VoxbloxMapBackend<SDFServerType, SDFVoxelType>::getVoxelStatus(
...
template <typename SDFServerType, typename SDFVoxelType>
VoxelStatus VoxbloxMapBackend<SDFServerType, SDFVoxelType>::getRayStatus(
...
template <typename SDFServerType, typename SDFVoxelType>
VoxelStatus VoxbloxMapBackend<SDFServerType, SDFVoxelType>::getBoxStatusInVoxels(
...
// [NEED TO REVIEW]
template <typename SDFServerType, typename SDFVoxelType>
float VoxbloxMapBackend<SDFServerType, SDFVoxelType>::getVoxelDistance(
...
// TSDF
template <typename SDFServerType, typename SDFVoxelType>
void VoxbloxMapBackend<SDFServerType, SDFVoxelType>::clearIfUnknown(
    SDFVoxelType& voxel) {
...
// ESDF
template <>
void VoxbloxMapBackend<voxblox::EsdfServer, voxblox::EsdfVoxel>::clearIfUnknown(
...
template <typename SDFServerType, typename SDFVoxelType>
bool VoxbloxMapBackend<SDFServerType, SDFVoxelType>::augmentFreeBox(
...
template <typename SDFServerType, typename SDFVoxelType>
double VoxbloxMapBackend<SDFServerType, SDFVoxelType>::getPointDistance(
...
template <typename SDFServerType, typename SDFVoxelType>
Eigen::Vector3d VoxbloxMapBackend<SDFServerType, SDFVoxelType>::getPointGradient(
  const Eigen::Vector3d& point) {
  //
...
// [NEED TO REVIEW]
template <typename SDFServerType, typename SDFVoxelType>
void VoxbloxMapBackend<SDFServerType, SDFVoxelType>::getFreeSpacePointCloud(
...
template <typename SDFServerType, typename SDFVoxelType>
void VoxbloxMapBackend<SDFServerType, SDFVoxelType>::augmentFreeFrustum() {
  ROS_WARN_THROTTLE(5.0, "VoxbloxMapBackend::augmentFreeFrustum: N/A");
}

template <typename SDFServerType, typename SDFVoxelType>
void VoxbloxMapBackend<SDFServerType, SDFVoxelType>::extractLocalMap(
    const Eigen::Vector3d& center, const Eigen::Vector3d& bounding_box_size,
    std::vector<Eigen::Vector3d>& occupied_voxels,
    std::vector<Eigen::Vector3d>& free_voxels) {
  // ROS_WARN_THROTTLE(5.0,
  //                   "VoxbloxMapBackend::extractLocalMap --> Temporary
...
template <typename SDFServerType, typename SDFVoxelType>
void VoxbloxMapBackend<SDFServerType, SDFVoxelType>::extractLocalMapAlongAxis(
...
template <typename SDFServerType, typename SDFVoxelType>
void VoxbloxMapBackend<SDFServerType, SDFVoxelType>::getLocalPointcloud(
...
template <typename SDFServerType, typename SDFVoxelType>
pcl::PointXYZI
VoxbloxMapBackend<SDFServerType, SDFVoxelType>::eigenVec3dToPCLPoint(
    Eigen::Vector3d& vec) {
  pcl::PointXYZI point;
  point.x = vec.x();
  point.y = vec.y();
...
template<typename SDFServerType, typename SDFVoxelType>
void VoxbloxMapBackend<SDFServerType, SDFVoxelType>::annotateCameraVoxels(Eigen::Vector3d& pos, std::vector<Eigen::Vector3d>& multiray_endpoints)
{
  const float voxel_size = sdf_layer_->voxel_size();
  const float voxel_size_inv = 1.0 / voxel_size;
...
template <typename SDFServerType, typename SDFVoxelType>
void VoxbloxMapBackend<SDFServerType, SDFVoxelType>::getCameraScanStatus(StateVec& state, SensorParamsBase& sensor_params, std::vector<VoxelLog> &out_logs)
{
...
template <typename SDFServerType, typename SDFVoxelType>
void VoxbloxMapBackend<SDFServerType, SDFVoxelType>::getCameraScanStatus(
