...
void CognitiveGraph::addVertex(Vertex* v) {
...
void CognitiveGraph::addEdge(Vertex* v, Vertex* u, double weight) {
...
void CognitiveGraph::removeEdge(Vertex* v, Vertex* u) {
...
bool CognitiveGraph::getNearestVertex(const StateVec* state, Vertex** v_res) {
...
bool CognitiveGraph::getNearestVertexInRange(const StateVec* state, double range,
                                           Vertex** v_res) {
  if (getNumVertices() <= 0) return false;
  kdres* nearest = kd_nearest3(kd_tree_, state->x(), state->y(), state->z());
  if (kd_res_size(nearest) <= 0) {
...
bool CognitiveGraph::getNearestVertices(const StateVec* state, double range,
...
bool CognitiveGraph::findShortestPaths(ShortestPathsReport& rep) {
  return graph_->findDijkstraShortestPaths(0, rep);
}

bool CognitiveGraph::findShortestPaths(int source_id, ShortestPathsReport& rep) {
  return graph_->findDijkstraShortestPaths(source_id, rep);
}

void CognitiveGraph::getShortestPath(int target_id,
...
double CognitiveGraph::getShortestDistance(int target_id,
                                         const ShortestPathsReport& rep) {
...
int CognitiveGraph::getParentIDFromShortestPath(int target_id,
                                              const ShortestPathsReport& rep) {
  if (!rep.status) {
    ROS_WARN_COND(global_verbosity >= Verbosity::WARN, "Shortest paths report is not valid");
    return target_id;
...
void CognitiveGraph::getLeafVertices(std::vector<Vertex*>& leaf_vertices) {
  leaf_vertices.clear();
...
void CognitiveGraph::findLeafVertices(const ShortestPathsReport& rep) {
...
int CognitiveGraph::generateSubgraphIndex() { return ++subgraph_ind_; }

int CognitiveGraph::generateVertexID() { return ++id_count_; }

void CognitiveGraph::updateVertexTypeInRange(StateVec& state, double range) {
...
void CognitiveGraph::convertGraphToMsg(planner_msgs::Graph& graph_msg) {
...
void CognitiveGraph::convertMsgToGraph(const planner_msgs::Graph& graph_msg) {
...
void CognitiveGraph::saveGraph(const std::string& path) {
...
void CognitiveGraph::loadGraph(const std::string& path) {
