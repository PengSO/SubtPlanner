...
namespace {
template <typename T>
bool readPlannerParam(const std::string& ns, const std::string& key,
                      const std::vector<std::string>& aliases, T& value,
                      std::string* resolved_key = nullptr) {
  const std::string primary = ns + "/" + key;
  if (ros::param::get(primary, value)) {
    if (resolved_key != nullptr) *resolved_key = primary;
    return true;
  }
  for (const std::string& alias : aliases) {
    const std::string candidate = ns + "/" + alias;
    if (ros::param::get(candidate, value)) {
      if (resolved_key != nullptr) *resolved_key = candidate;
      return true;
    }
  }
  if (resolved_key != nullptr) *resolved_key = primary;
...
bool BoundedSpaceParams::loadParams(std::string ns) {
  ...
  else if (!parse_str.compare("kCylinder"))
    type = BoundedSpaceType::kCylinder;
  else {
    ROSPARAM_ERROR(param_name);
    return false;
  }
  ...
  if (type == BoundedSpaceType::kCuboid ||
    ...
    if (type == BoundedSpaceType::kCylinder) {
      param_name = ns + "/radius";
      if (!ros::param::get(param_name, radius)) {
        radius = std::min(std::abs(min_val[0]), std::abs(max_val[0]));
        ROSPARAM_WARN(param_name, std::to_string(radius) + "m");
      }
...
bool BoundedSpaceParams::isInsideSpace(Eigen::Vector3d& pos) {
  ...
  } else if (type == BoundedSpaceType::kCylinder) {
    Eigen::Vector3d pos_B = rot_B2W * (pos - root_pos);
    const double xy_dist = std::hypot(pos_B[0], pos_B[1]);
    if (xy_dist > radius_total ||
        pos_B[2] < min_val_total[2] || pos_B[2] > max_val_total[2]) {
      res = false;
    }
...
bool PlanningParams::loadParams(std::string ns) {
  ...
  parse_str = "";
  param_name.clear();
  readPlannerParam(ns, "exploration_graph_update_mode", {"graph_building_mode"},
                   parse_str, &param_name);
  if (!parse_str.compare("kBatch"))
    ...
    graph_building_mode = GraphBuildingModeType::kBasic;
    ROSPARAM_WARN(param_name, "kBasic");
  }

  parse_str = "";
  param_name.clear();
  readPlannerParam(ns, "exploration_graph_mode", {"rr_mode"}, parse_str,
                   &param_name);
  if (!parse_str.compare("kTree"))
    ...
    rr_mode = RRModeType::kGraph;
    ROSPARAM_WARN(param_name, "kGraph");
  }

  std::vector<std::string> parse_str_list;
  readPlannerParam(ns, "exploration_sensor_list", {"exp_sensor_list"},
                   parse_str_list, &param_name);
  if (parse_str_list.size() <= 0) {
  ...
  if (!readPlannerParam(ns, "exploration_vertex_budget", {"num_vertices_max"},
                        num_vertices_max, &param_name)) {
    num_vertices_max = 500;
    ROSPARAM_WARN(param_name, num_vertices_max);
  }

  if (!readPlannerParam(ns, "exploration_edge_budget", {"num_edges_max"},
                        num_edges_max, &param_name)) {
    num_edges_max = 5000;
  ...
  if (!readPlannerParam(ns, "topological_anchor_z_offset", {},
                        topological_anchor_z_offset, &param_name)) {
    topological_anchor_z_offset = 0.2;
    ROSPARAM_WARN(param_name, topological_anchor_z_offset);
  }
  ...
  param_name = ns + "/local_path_smoothing_enable";
  if (!ros::param::get(param_name, local_path_smoothing_enable)) {
    local_path_smoothing_enable = false;
    ROSPARAM_WARN(param_name, local_path_smoothing_enable);
  }

  param_name = ns + "/local_path_smoothing_iterations";
  if (!ros::param::get(param_name, local_path_smoothing_iterations)) {
    local_path_smoothing_iterations = 1;
    ROSPARAM_WARN(param_name, local_path_smoothing_iterations);
  }

  param_name = ns + "/local_path_smoothing_min_segment";
  if (!ros::param::get(param_name, local_path_smoothing_min_segment)) {
    local_path_smoothing_min_segment = 0.2;
    ROSPARAM_WARN(param_name, local_path_smoothing_min_segment);
  }
  ...
  if (!readPlannerParam(ns, "topological_frontiers_only",
                        {"add_only_frontiers_to_global_graph"},
                        add_only_frontiers_to_global_graph, &param_name)) {
  ...
  if (!readPlannerParam(ns, "topological_node_spacing",
                        {"global_graph_odom_dist"}, global_graph_odom_dist,
                        &param_name)) {
    global_graph_odom_dist = 0.5;
    ROSPARAM_WARN(param_name, global_graph_odom_dist);
  }
  
  if (!readPlannerParam(ns, "topological_connection_radius",
                        {"global_graph_odom_connect_radius"},
                        global_graph_odom_connect_radius, &param_name)) {
  ...
  param_name = ns + "/local_navigation_min_clearance";
  if (!ros::param::get(param_name, local_navigation_min_clearance)) {
    local_navigation_min_clearance = 0.35;
    ROSPARAM_WARN(param_name, local_navigation_min_clearance);
  }

  param_name = ns + "/local_navigation_clearance_weight";
  if (!ros::param::get(param_name, local_navigation_clearance_weight)) {
    local_navigation_clearance_weight = 0.35;
    ROSPARAM_WARN(param_name, local_navigation_clearance_weight);
  }

  param_name = ns + "/local_navigation_min_path_length";
  if (!ros::param::get(param_name, local_navigation_min_path_length)) {
    local_navigation_min_path_length = 0.4;
    ROSPARAM_WARN(param_name, local_navigation_min_path_length);
  }
  ...
  param_name = ns + "/local_navigation_recovery_enable";
  if (!ros::param::get(param_name, local_navigation_recovery_enable)) {
    local_navigation_recovery_enable = true;
    ROSPARAM_WARN(param_name, local_navigation_recovery_enable);
  }

  param_name = ns + "/local_navigation_recovery_max_attempts";
  if (!ros::param::get(param_name, local_navigation_recovery_max_attempts)) {
    local_navigation_recovery_max_attempts = 3;
    ROSPARAM_WARN(param_name, local_navigation_recovery_max_attempts);
  }

  param_name = ns + "/local_navigation_recovery_distance";
  if (!ros::param::get(param_name, local_navigation_recovery_distance)) {
    local_navigation_recovery_distance = edge_length_max;
    ROSPARAM_WARN(param_name, local_navigation_recovery_distance);
  }
  ...
  if (!readPlannerParam(ns, "passage_guided_sampling_enable",
                        {"axis_guided_sampling_enable"},
                        axis_guided_sampling_enable, &param_name)) {
    axis_guided_sampling_enable = true;
    ROSPARAM_WARN(param_name, axis_guided_sampling_enable);
  }

  if (!readPlannerParam(ns, "disable_passage_guided_sampling",
                        {"disable_local_sample_guidance"},
                        disable_local_sample_guidance, &param_name)) {
    disable_local_sample_guidance = false;
    ROSPARAM_WARN(param_name, disable_local_sample_guidance);
  }

  if (!readPlannerParam(ns, "directional_candidate_batch_size",
                        {"sample_candidate_batch_size"},
                        sample_candidate_batch_size, &param_name)) {
    sample_candidate_batch_size = 2;
    ROSPARAM_WARN(param_name, sample_candidate_batch_size);
  }

  if (!readPlannerParam(ns, "passage_axis_sampling_ratio",
                        {"axis_biased_sampling_ratio"},
                        axis_biased_sampling_ratio, &param_name)) {
    axis_biased_sampling_ratio = 0.0;
    ROSPARAM_WARN(param_name, axis_biased_sampling_ratio);
  }

  if (!readPlannerParam(ns, "passage_lateral_sampling_scale",
                        {"axis_biased_lateral_scale"},
                        axis_biased_lateral_scale, &param_name)) {
    axis_biased_lateral_scale = 0.6;
    ROSPARAM_WARN(param_name, axis_biased_lateral_scale);
  }

  if (!readPlannerParam(ns, "passage_backward_sampling_scale",
                        {"axis_biased_backward_scale"},
                        axis_biased_backward_scale, &param_name)) {
    axis_biased_backward_scale = 0.4;
    ROSPARAM_WARN(param_name, axis_biased_backward_scale);
  }

  param_name = ns + "/sample_axis_alignment_weight";
  if (!ros::param::get(param_name, sample_axis_alignment_weight)) {
    sample_axis_alignment_weight = 0.4;
    ROSPARAM_WARN(param_name, sample_axis_alignment_weight);
  }

  param_name = ns + "/sample_clearance_weight";
  if (!ros::param::get(param_name, sample_clearance_weight)) {
    sample_clearance_weight = 0.6;
    ROSPARAM_WARN(param_name, sample_clearance_weight);
  }

  param_name = ns + "/sample_goal_alignment_weight";
  if (!ros::param::get(param_name, sample_goal_alignment_weight)) {
    sample_goal_alignment_weight = 0.25;
    ROSPARAM_WARN(param_name, sample_goal_alignment_weight);
  }

  param_name = ns + "/sample_same_zone_bonus";
  if (!ros::param::get(param_name, sample_same_zone_bonus)) {
    sample_same_zone_bonus = 0.2;
    ROSPARAM_WARN(param_name, sample_same_zone_bonus);
  }

  param_name = ns + "/opening_frontier_seed_enable";
  if (!ros::param::get(param_name, opening_frontier_seed_enable)) {
    opening_frontier_seed_enable = false;
    ROSPARAM_WARN(param_name, opening_frontier_seed_enable);
  }

  param_name = ns + "/opening_frontier_seed_max_num";
  if (!ros::param::get(param_name, opening_frontier_seed_max_num)) {
    opening_frontier_seed_max_num = 3;
    ROSPARAM_WARN(param_name, opening_frontier_seed_max_num);
  }

  param_name = ns + "/opening_frontier_seed_angle_bins";
  if (!ros::param::get(param_name, opening_frontier_seed_angle_bins)) {
    opening_frontier_seed_angle_bins = 24;
    ROSPARAM_WARN(param_name, opening_frontier_seed_angle_bins);
  }

  param_name = ns + "/opening_frontier_seed_min_free_length";
  if (!ros::param::get(param_name, opening_frontier_seed_min_free_length)) {
    opening_frontier_seed_min_free_length =
        std::max(1.0, 2.0 * edge_length_min);
    ROSPARAM_WARN(param_name, opening_frontier_seed_min_free_length);
  }

  param_name = ns + "/opening_frontier_seed_unknown_depth";
  if (!ros::param::get(param_name, opening_frontier_seed_unknown_depth)) {
    opening_frontier_seed_unknown_depth =
        std::max(1.0, edge_length_max);
    ROSPARAM_WARN(param_name, opening_frontier_seed_unknown_depth);
  }

  param_name = ns + "/opening_frontier_seed_min_angle_sep";
  if (!ros::param::get(param_name, opening_frontier_seed_min_angle_sep)) {
    opening_frontier_seed_min_angle_sep = M_PI / 6.0;
    ROSPARAM_WARN(param_name, opening_frontier_seed_min_angle_sep);
  }

  param_name = ns + "/opening_frontier_seed_min_clearance";
  if (!ros::param::get(param_name, opening_frontier_seed_min_clearance)) {
    opening_frontier_seed_min_clearance =
        std::max(local_navigation_min_clearance, 0.35);
    ROSPARAM_WARN(param_name, opening_frontier_seed_min_clearance);
  }

  param_name = ns + "/opening_frontier_seed_search_radius";
  if (!ros::param::get(param_name, opening_frontier_seed_search_radius)) {
    opening_frontier_seed_search_radius =
        std::max(2.0 * edge_length_max, 6.0);
    ROSPARAM_WARN(param_name, opening_frontier_seed_search_radius);
  }

  param_name = ns + "/fine_gain_evaluation_enable";
  if (!ros::param::get(param_name, fine_gain_evaluation_enable)) {
    fine_gain_evaluation_enable = true;
    ROSPARAM_WARN(param_name, fine_gain_evaluation_enable);
  }

  param_name = ns + "/fine_gain_top_k";
  if (!ros::param::get(param_name, fine_gain_top_k)) {
    fine_gain_top_k = 80;
    ROSPARAM_WARN(param_name, fine_gain_top_k);
  }

  param_name = ns + "/fine_gain_candidate_ratio";
  if (!ros::param::get(param_name, fine_gain_candidate_ratio)) {
    fine_gain_candidate_ratio = 0.4;
    ROSPARAM_WARN(param_name, fine_gain_candidate_ratio);
  }

  param_name = ns + "/zone_partition_enable";
  if (!ros::param::get(param_name, zone_partition_enable)) {
    zone_partition_enable = true;
    ROSPARAM_WARN(param_name, zone_partition_enable);
  }

  param_name = ns + "/zone_partition_rows";
  if (!ros::param::get(param_name, zone_partition_rows)) {
    zone_partition_rows = 2;
    ROSPARAM_WARN(param_name, zone_partition_rows);
  }

  param_name = ns + "/zone_partition_cols";
  if (!ros::param::get(param_name, zone_partition_cols)) {
    zone_partition_cols = 2;
    ROSPARAM_WARN(param_name, zone_partition_cols);
  }

  param_name = ns + "/zone_cross_penalty";
  if (!ros::param::get(param_name, zone_cross_penalty)) {
    zone_cross_penalty = 0.25;
    ROSPARAM_WARN(param_name, zone_cross_penalty);
  }

  param_name = ns + "/zone_same_region_bonus";
  if (!ros::param::get(param_name, zone_same_region_bonus)) {
    zone_same_region_bonus = 0.1;
    ROSPARAM_WARN(param_name, zone_same_region_bonus);
  }

  if (!readPlannerParam(ns, "repositioning_goal_hysteresis_enable",
                        {"global_frontier_stickiness_enable"},
                        global_frontier_stickiness_enable, &param_name)) {
    global_frontier_stickiness_enable = true;
    ROSPARAM_WARN(param_name, global_frontier_stickiness_enable);
  }

  if (!readPlannerParam(ns, "repositioning_goal_hysteresis_ratio",
                        {"global_frontier_stickiness_ratio"},
                        global_frontier_stickiness_ratio, &param_name)) {
    global_frontier_stickiness_ratio = 1.15;
    ROSPARAM_WARN(param_name, global_frontier_stickiness_ratio);
  }

  if (!readPlannerParam(ns, "branch_aware_repositioning_enable",
                        {},
                        branch_repositioning_selection_enable, &param_name)) {
    branch_repositioning_selection_enable = true;
    ROSPARAM_WARN(param_name, branch_repositioning_selection_enable);
  }

  if (!readPlannerParam(ns, "branch_region_selection_count",
                        {},
                        branch_repositioning_region_count, &param_name)) {
    branch_repositioning_region_count = 2;
    ROSPARAM_WARN(param_name, branch_repositioning_region_count);
  }

  if (!readPlannerParam(ns, "branch_memory_alpha",
                        {},
                        branch_repositioning_memory_alpha, &param_name)) {
    branch_repositioning_memory_alpha = 0.25;
    ROSPARAM_WARN(param_name, branch_repositioning_memory_alpha);
  }

  if (!readPlannerParam(ns, "branch_frontier_scale_weight",
                        {},
                        branch_repositioning_frontier_scale_weight, &param_name)) {
    branch_repositioning_frontier_scale_weight = 1.0;
    ROSPARAM_WARN(param_name, branch_repositioning_frontier_scale_weight);
  }

  if (!readPlannerParam(ns, "branch_compactness_weight",
                        {},
                        branch_repositioning_compactness_weight, &param_name)) {
    branch_repositioning_compactness_weight = 1.0;
    ROSPARAM_WARN(param_name, branch_repositioning_compactness_weight);
  }

  if (!readPlannerParam(ns, "branch_success_memory_weight",
                        {},
                        branch_repositioning_success_weight, &param_name)) {
    branch_repositioning_success_weight = 1.0;
    ROSPARAM_WARN(param_name, branch_repositioning_success_weight);
  }

  if (!readPlannerParam(ns, "repositioning_time_weight",
                        {},
                        branch_repositioning_time_weight, &param_name)) {
    branch_repositioning_time_weight = 1.0;
    ROSPARAM_WARN(param_name, branch_repositioning_time_weight);
  }

  param_name = ns + "/dead_zone_escape_enable";
  if (!ros::param::get(param_name, dead_zone_escape_enable)) {
    dead_zone_escape_enable = true;
    ROSPARAM_WARN(param_name, dead_zone_escape_enable);
  }

  param_name = ns + "/dead_zone_min_low_gain_iters";
  if (!ros::param::get(param_name, dead_zone_min_low_gain_iters)) {
    dead_zone_min_low_gain_iters = max_num_low_gain_iters;
    ROSPARAM_WARN(param_name, dead_zone_min_low_gain_iters);
  }

  if (!readPlannerParam(ns, "dead_zone_max_branch_degree",
                        {},
                        dead_zone_max_branch_degree, &param_name)) {
    dead_zone_max_branch_degree = 1;
    ROSPARAM_WARN(param_name, dead_zone_max_branch_degree);
  }

  param_name = ns + "/dead_zone_front_clearance";
  if (!ros::param::get(param_name, dead_zone_front_clearance)) {
    dead_zone_front_clearance = std::max(1.0, edge_length_min);
    ROSPARAM_WARN(param_name, dead_zone_front_clearance);
  }

  param_name = ns + "/dead_zone_region_radius";
  if (!ros::param::get(param_name, dead_zone_region_radius)) {
    dead_zone_region_radius = std::max(2.0, edge_length_max);
    ROSPARAM_WARN(param_name, dead_zone_region_radius);
  }

  param_name = ns + "/dead_zone_penalty_lambda";
  if (!ros::param::get(param_name, dead_zone_penalty_lambda)) {
    dead_zone_penalty_lambda = 0.2;
    ROSPARAM_WARN(param_name, dead_zone_penalty_lambda);
  }

  param_name = ns + "/dead_zone_turn_radius";
  if (!ros::param::get(param_name, dead_zone_turn_radius)) {
    dead_zone_turn_radius = 1.0;
    ROSPARAM_WARN(param_name, dead_zone_turn_radius);
  }

  param_name = ns + "/dead_zone_turn_clearance";
  if (!ros::param::get(param_name, dead_zone_turn_clearance)) {
    dead_zone_turn_clearance = std::max(local_navigation_min_clearance, 0.35);
    ROSPARAM_WARN(param_name, dead_zone_turn_clearance);
  }

  param_name = ns + "/dead_zone_turn_step";
  if (!ros::param::get(param_name, dead_zone_turn_step)) {
    dead_zone_turn_step = 0.5;
    ROSPARAM_WARN(param_name, dead_zone_turn_step);
  }

  param_name = ns + "/dead_zone_escape_prefix_length";
  if (!ros::param::get(param_name, dead_zone_escape_prefix_length)) {
    dead_zone_escape_prefix_length = std::max(1.0, edge_length_min);
    ROSPARAM_WARN(param_name, dead_zone_escape_prefix_length);
  }

  param_name = ns + "/shortcut_edges_enable";
  if (!ros::param::get(param_name, shortcut_edges_enable)) {
    shortcut_edges_enable = true;
    ROSPARAM_WARN(param_name, shortcut_edges_enable);
  }

  param_name = ns + "/shortcut_edge_radius";
  if (!ros::param::get(param_name, shortcut_edge_radius)) {
    shortcut_edge_radius = 4.0;
    ROSPARAM_WARN(param_name, shortcut_edge_radius);
  }

  param_name = ns + "/shortcut_detour_ratio";
  if (!ros::param::get(param_name, shortcut_detour_ratio)) {
    shortcut_detour_ratio = 1.4;
    ROSPARAM_WARN(param_name, shortcut_detour_ratio);
  }

  param_name = ns + "/shortcut_min_clearance";
  if (!ros::param::get(param_name, shortcut_min_clearance)) {
    shortcut_min_clearance = 0.25;
    ROSPARAM_WARN(param_name, shortcut_min_clearance);
  }
