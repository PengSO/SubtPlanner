#ifndef SUBT_PLANNER_VISUALIZATION_MISSION_VISUALS_H_
#define SUBT_PLANNER_VISUALIZATION_MISSION_VISUALS_H_
...
class MissionVisuals {
 public:
  MissionVisuals(const ros::NodeHandle& nh, const ros::NodeHandle& nh_private);
  // Render the active cognitive-map workspace bounds.
  void visualizeWorkspace(StateVec& state, BoundedSpaceParams& global_ws,
                          BoundedSpaceParams& local_ws);
  // Render dead-zone regions used by global repositioning
  void visualizeNoGainZones(std::vector<BoundedSpaceParams>& no_gain_zones);
  // Render the local exploration graph.
  void visualizeGraph(const std::shared_ptr<CognitiveGraph> graph_manager);
  // Visualize subset of graph vertices
  void visualizeGraphVertices(const std::shared_ptr<CognitiveGraph> graph_manager, const std::vector<int> &ids);
  // Render the planar projection of the exploration graph.
  ...
  void visualizeShortestPaths(const std::shared_ptr<CognitiveGraph> graph_manager,
                              const ShortestPathsReport& graph_rep);
  // Visualize paths rejected by geometric consistency checks.
  void visualizeNegativePaths(const std::vector<int>& ids,
                              const std::shared_ptr<CognitiveGraph> graph_manager,
                              const ShortestPathsReport& graph_rep);
  // Visualize rejected graph edges.
  void visualizeNegativePaths(const std::vector<Eigen::Vector3d>& edge_vertices,
                              const std::shared_ptr<CognitiveGraph> graph_manager,
                              const ShortestPathsReport& graph_rep);

  // Visualize best path[s].
  void visualizeBestPaths(const std::shared_ptr<CognitiveGraph> graph_manager,
  ...
  // Render the cognitive-map topological graph.
  void visualizeTopologicalGraph(const std::shared_ptr<CognitiveGraph> graph_manager);
  // Render the return path.
  void visualizeHomingPath(const std::shared_ptr<CognitiveGraph> graph_manager,
                           const ShortestPathsReport& graph_rep,
                           int current_id);
  ...
  // Render global repositioning paths.
  void visualizeGlobalPaths(const std::shared_ptr<CognitiveGraph> graph_manager,
  ...
  void visualizePath(const std::shared_ptr<CognitiveGraph> graph_manager,
 ...
 private:
  ...
  ros::Publisher dead_zone_region_pub_;
  ros::Publisher robot_state_pub_;
  ros::Publisher exploration_graph_pub_;
  ...
  ros::Publisher selected_paths_pub_;
  ros::Publisher local_reference_path_pub_;
  ros::Publisher ref_path_color_pub_;
  ros::Publisher mod_path_pub_;
  ros::Publisher blind_mod_path_pub_;
  ros::Publisher topological_graph_pub_;
  ros::Publisher return_path_pub_;
  ros::Publisher repositioning_path_pub_;
