...
#include "subt_planner_visualization/mission_visuals.h"

#include <tf/transform_datatypes.h>

MissionVisuals::MissionVisuals(const ros::NodeHandle& nh,
                             const ros::NodeHandle& nh_private) {
  nh_ = nh;
  nh_private_ = nh_private;

  planning_workspace_pub_ = nh_.advertise<visualization_msgs::MarkerArray>(
      "vis/cognitive_map/workspace", 10);
  dead_zone_region_pub_ =
      nh_.advertise<visualization_msgs::MarkerArray>("vis/global_repositioning/dead_zone_regions", 10);
  exploration_graph_pub_ =
      nh_.advertise<visualization_msgs::MarkerArray>("vis/local_exploration/exploration_graph", 10);
  planning_projected_graph_pub_ =
      nh_.advertise<visualization_msgs::MarkerArray>(
          "vis/local_exploration/projected_graph", 10);
  planning_failed_pub_ =
      nh_.advertise<visualization_msgs::MarkerArray>("vis/local_exploration/rejected_edges", 10);
  shortest_paths_pub_ =
      nh_.advertise<visualization_msgs::MarkerArray>("vis/local_exploration/reachable_paths", 10);
  robot_state_pub_ =
  ...
  selected_paths_pub_ = nh_.advertise<visualization_msgs::MarkerArray>(
      "vis/local_exploration/selected_paths", 10);
  volumetric_gain_pub_ = nh_.advertise<visualization_msgs::MarkerArray>(
      "vis/local_exploration/volumetric_gain", 10);
  sampler_pub_ =
      nh_.advertise<visualization_msgs::MarkerArray>("vis/local_exploration/sampler", 10);
  rays_pub_ =
      nh_.advertise<visualization_msgs::MarkerArray>("vis/local_exploration/ray_casting", 10);
  local_reference_path_pub_ =
      nh_.advertise<visualization_msgs::MarkerArray>("vis/local_exploration/reference_path", 10);
  ref_path_color_pub_ =
      nh_.advertise<visualization_msgs::MarkerArray>("vis/local_exploration/colored_reference_path", 10);
  topological_graph_pub_ = nh_.advertise<visualization_msgs::MarkerArray>(
      "vis/cognitive_map/topological_graph", 10);
  return_path_pub_ = nh_.advertise<visualization_msgs::MarkerArray>(
      "vis/global_repositioning/return_path", 10);
  repositioning_path_pub_ = nh_.advertise<visualization_msgs::MarkerArray>(
      "vis/global_repositioning/repositioning_path", 10);
  clustered_paths_pub_ = nh_.advertise<visualization_msgs::MarkerArray>(
      "vis/local_exploration/path_clusters", 10);
  state_history_pub_ =
      nh_.advertise<visualization_msgs::MarkerArray>("vis/cognitive_map/state_history", 10);
  geofence_pub_ =
      nh_.advertise<visualization_msgs::MarkerArray>("vis/cognitive_map/geofence", 10);
  negative_edges_pub_ =
      nh_.advertise<visualization_msgs::MarkerArray>("vis/local_exploration/negative_edges", 10);
  ...
  pcl_pub_ = nh_.advertise<sensor_msgs::PointCloud2>("vis/cognitive_map/occupied_cloud", 10);
  path_pub_ =
      nh_.advertise<visualization_msgs::MarkerArray>("vis/local_exploration/alternate_path", 10);
  opening_traversal_path_pub_ =
      nh_.advertise<visualization_msgs::MarkerArray>("vis/local_exploration/opening_traversal_path", 10);
  graph_vertices_pub_ =
      nh_.advertise<visualization_msgs::MarkerArray>("vis/local_exploration/graph_vertices", 10);
  viewpoints_pub_ =
      nh_.advertise<geometry_msgs::PoseArray>("vis/local_exploration/viewpoints", 10);

  best_path_id_ = 0;
}

void MissionVisuals::visualizeWorkspace(StateVec& state,
  ...
  } else if (global_ws.type == BoundedSpaceType::kCylinder) {
    global_ws_marker.type = visualization_msgs::Marker::CYLINDER;
    global_ws_marker.pose.position.x = global_ws.getCenter()(0);
    global_ws_marker.pose.position.y = global_ws.getCenter()(1);
    global_ws_marker.pose.position.z =
        global_ws.getCenter()(2) +
        0.5 * (global_ws.min_val(2) + global_ws.max_val(2));
    global_ws_marker.scale.x = global_ws.radius * 2;
  ...
  } else if (local_ws.type == BoundedSpaceType::kCylinder) {
    local_ws_marker.type = visualization_msgs::Marker::CYLINDER;
    local_ws_marker.pose.position.x = state[0];
    local_ws_marker.pose.position.y = state[1];
...
void MissionVisuals::visualizeNoGainZones(
    std::vector<BoundedSpaceParams>& no_gain_zones) {
  if (dead_zone_region_pub_.getNumSubscribers() < 1) return;
  visualization_msgs::MarkerArray marker_array;
  ...
  for (int i = 0; i < no_gain_zones.size(); ++i) {
    ...
    } else if (no_gain_zone.type == BoundedSpaceType::kCylinder) {
      no_gain_zone_marker.type = visualization_msgs::Marker::CYLINDER;
      no_gain_zone_marker.pose.position.x = no_gain_zone.getCenter()(0);
      no_gain_zone_marker.pose.position.y = no_gain_zone.getCenter()(1);
      no_gain_zone_marker.pose.position.z =
          no_gain_zone.getCenter()(2) +
          0.5 * (no_gain_zone.min_val(2) + no_gain_zone.max_val(2));
      no_gain_zone_marker.scale.x = no_gain_zone.radius * 2;
  ...
  dead_zone_region_pub_.publish(marker_array);
}

void MissionVisuals::visualizeGraphVertices(const std::shared_ptr<CognitiveGraph> graph_manager, const std::vector<int> &ids) {
  visualization_msgs::MarkerArray marker_array;
...
void MissionVisuals::visualizeGraph(
  ...
  if (exploration_graph_pub_.getNumSubscribers() < 1) return;
  ...
  edge_marker.color.g = 210.0 / 255.0;
  ...
  vertex_marker.color.r = 20.0 / 255.0;
  vertex_marker.color.g = 130.0 / 255.0;
  vertex_marker.color.b = 255.0 / 255.0;
  ...
  hanging_vertex_marker.color.g = 0.0;
  hanging_vertex_marker.color.b = 210.0 / 255.0;
  ...
  exploration_graph_pub_.publish(marker_array);
}

void MissionVisuals::visualizeProjectedGraph(
    const std::shared_ptr<CognitiveGraph> graph_manager) {
  std::shared_ptr<Graph> g = graph_manager->graph_;
  std::unordered_map<int, Vertex*>& v_map = graph_manager->vertices_map_;
  ...
  edge_marker.scale.x = 0.04;
  edge_marker.color.r = 0.0;
  edge_marker.color.g = 200.0 / 255.0;
  edge_marker.color.b = 255.0 / 255.0;
  edge_marker.color.a = 1.0;
  edge_marker.lifetime = ros::Duration(graph_lifetime);
  edge_marker.frame_locked = false;
  ...
  vertex_marker.scale.y = 0.3;
  vertex_marker.scale.z = 0.3;
  vertex_marker.color.r = 70.0 / 255.0;
...
void MissionVisuals::visualizeTopologicalGraph(
    const std::shared_ptr<CognitiveGraph> graph_manager) {
  std::shared_ptr<Graph> g = graph_manager->graph_;
  std::unordered_map<int, Vertex*>& v_map = graph_manager->vertices_map_;

  if (graph_manager->getNumVertices() == 0) return;
  if (topological_graph_pub_.getNumSubscribers() < 1) return;
  ...
  edge_marker.color.r = 0.0;
  edge_marker.color.g = 190.0 / 255.0;
  edge_marker.color.b = 255.0 / 255.0;
  edge_marker.color.a = 1.0;
  edge_marker.lifetime = ros::Duration(graph_lifetime);
  edge_marker.frame_locked = false;
  ...
  vertex_marker.color.r = 40.0 / 255.0;
  vertex_marker.color.g = 120.0 / 255.0;
  vertex_marker.color.b = 255.0 / 255.0;
  vertex_marker.color.a = 1.0;
  vertex_marker.lifetime = ros::Duration(graph_lifetime);
  vertex_marker.frame_locked = false;
  ...
  frontier_marker.color.r = 255.0 / 255.0;
  frontier_marker.color.g = 0.0;
  frontier_marker.color.b = 255.0 / 255.0;
  ...
  semantic_marker_stairs.color.r = 255.0 / 255.0;
  semantic_marker_stairs.color.g = 120.0 / 255.0;
  ...
  semantic_marker_door.color.r = 155.0 / 255.0;
  semantic_marker_door.color.g = 80.0 / 255.0;
  semantic_marker_door.color.b = 255.0 / 255.0;
  ...
  topological_graph_pub_.publish(marker_array);
}

void MissionVisuals::visualizeFailedEdges(std::shared_ptr<SampleStatistic> ss) {
...
void MissionVisuals::visualizeRobotState(StateVec& state,
                                        RobotParams& robot_params) {
  if (robot_state_pub_.getNumSubscribers() < 1) return;
  visualization_msgs::MarkerArray marker_array;

  visualization_msgs::Marker delete_size_marker;
  delete_size_marker.header.stamp = ros::Time::now();
  delete_size_marker.header.seq = 0;
  delete_size_marker.header.frame_id = world_frame_id;
  delete_size_marker.id = 0;
  delete_size_marker.ns = "size";
  delete_size_marker.action = visualization_msgs::Marker::DELETE;
  marker_array.markers.push_back(delete_size_marker);

  visualization_msgs::Marker delete_extension_aligned_marker = delete_size_marker;
  delete_extension_aligned_marker.ns = "extension_aligned";
  marker_array.markers.push_back(delete_extension_aligned_marker);
  ...
  robot_size_marker.ns = "model";
  robot_size_marker.action = visualization_msgs::Marker::ADD;
  robot_size_marker.type = visualization_msgs::Marker::MESH_RESOURCE;
  robot_size_marker.mesh_resource =
      "package://rotors_description/meshes/rmf_obelix/rmf_obelix.dae";
  robot_size_marker.mesh_use_embedded_materials = true;
  robot_size_marker.color.r = 1.0;
  robot_size_marker.color.g = 1.0;
  robot_size_marker.color.b = 1.0;
  robot_size_marker.color.a = 1.0;
  robot_size_marker.scale.x = 1.0;
  robot_size_marker.scale.y = 1.0;
  robot_size_marker.scale.z = 1.0;
  ...
  robot_esize_marker.ns = "planning_box";
  robot_esize_marker.action = visualization_msgs::Marker::ADD;
  robot_esize_marker.type = visualization_msgs::Marker::LINE_LIST;
  robot_esize_marker.color.r = 0.0;
  robot_esize_marker.color.g = 0.0;
  robot_esize_marker.color.b = 0.0;
  robot_esize_marker.color.a = 0.9;
  Eigen::Vector3d ext_size;
  robot_params.getPlanningSize(ext_size);
  robot_esize_marker.scale.x = 0.04;
  robot_esize_marker.scale.y = 0.04;
  robot_esize_marker.scale.z = 0.04;
  ...
  const double hx = 0.5 * ext_size[0];
  const double hy = 0.5 * ext_size[1];
  const double hz = 0.5 * ext_size[2];
  corners[0].x = -hx; corners[0].y = -hy; corners[0].z = -hz;
  corners[1].x =  hx; corners[1].y = -hy; corners[1].z = -hz;
  corners[2].x =  hx; corners[2].y =  hy; corners[2].z = -hz;
  corners[3].x = -hx; corners[3].y =  hy; corners[3].z = -hz;
  corners[4].x = -hx; corners[4].y = -hy; corners[4].z =  hz;
  corners[5].x =  hx; corners[5].y = -hy; corners[5].z =  hz;
  corners[6].x =  hx; corners[6].y =  hy; corners[6].z =  hz;
  corners[7].x = -hx; corners[7].y =  hy; corners[7].z =  hz;
  const int edge_indices[24] = {0, 1, 1, 2, 2, 3, 3, 0,
                                4, 5, 5, 6, 6, 7, 7, 4,
                                0, 4, 1, 5, 2, 6, 3, 7};
  for (int i = 0; i < 24; ++i) {
    robot_esize_marker.points.push_back(corners[edge_indices[i]]);
  }
  ...
  robot_esize_xyz_marker.action = visualization_msgs::Marker::DELETE;
...
void MissionVisuals::visualizeSensorFOV(StateVec& state,
  ...
  for (int i = 0; i < sensor_params.sensor_list.size(); ++i) {
    ...
    if (sb->type == SensorType::kCamera) {
      ...
      line_marker.color.r = 180.0 / 255.0;
      line_marker.color.g = 220.0 / 255.0;
      ...
      cyl_marker.scale.x = sb->max_range * 2;
      cyl_marker.scale.y = sb->max_range * 2;
      cyl_marker.scale.z = sb->max_range * sin(sb->fov[1] / 2) * 2;
      ...
      Eigen::Matrix3d rot_W2S = rot_W2B * rot_B2S;
      ...
      cyl_marker.color.r = 180.0 / 255.0;
      cyl_marker.color.g = 220.0 / 255.0;
      cyl_marker.color.b = 1.0;
      cyl_marker.color.a = 0.2;
...
void MissionVisuals::visualizeNegativePaths(
...
void MissionVisuals::visualizeShortestPaths(
    const std::shared_ptr<CognitiveGraph> graph_manager,
    const ShortestPathsReport& graph_rep) {
  std::shared_ptr<Graph> g = graph_manager->graph_;
  std::unordered_map<int, Vertex*>& v_map = graph_manager->vertices_map_;
  ...
  edge_marker.color.r = 120.0 / 255.0;
  edge_marker.color.g = 80.0 / 255.0;
  edge_marker.color.b = 255.0 / 255.0;
  edge_marker.color.a = 1.0;
  edge_marker.lifetime = ros::Duration(shortest_paths_lifetime);
  edge_marker.frame_locked = false;
  ...
  vertex_marker.color.g = 145.0 / 255.0;
  ...
  leaf_vertex_marker.color.r = 255.0 / 255.0;
  leaf_vertex_marker.color.g = 90.0 / 255.0;
  ...
  best_leaf_marker.color.g = 255.0 / 255.0;
  best_leaf_marker.color.b = 255.0 / 255.0;
...
void MissionVisuals::visualizeClusteredPaths(
    const std::shared_ptr<CognitiveGraph> graph_manager,
...
bool MissionVisuals::getHeatMapColor(float value, float& red, float& green,
                                    float& blue) {
...
void MissionVisuals::visualizeHomingPath(
    const std::shared_ptr<CognitiveGraph> graph_manager,
    const ShortestPathsReport& graph_rep, int current_id) {
  if (return_path_pub_.getNumSubscribers() < 1) return;
  ...
  marker_array.markers.push_back(edge_marker);
  return_path_pub_.publish(marker_array);
}

void MissionVisuals::visualizeGlobalPaths(
    const std::shared_ptr<CognitiveGraph> graph_manager,
    std::vector<int>& to_frontier_ids, std::vector<int>& to_home_ids) {
  if (repositioning_path_pub_.getNumSubscribers() < 1) return;
  ...
  frontier_marker.color.r = 255.0 / 255.0;
  frontier_marker.color.g = 0.0;
  frontier_marker.color.b = 220.0 / 255.0;
  frontier_marker.color.a = 1.0;
  frontier_marker.lifetime = ros::Duration(shortest_paths_lifetime);
  frontier_marker.frame_locked = false;
  ...
  home_marker.color.r = 255.0 / 255.0;
  home_marker.color.g = 135.0 / 255.0;
  ...
  repositioning_path_pub_.publish(marker_array);
}

void MissionVisuals::visualizeOpeningTraversalPath(const std::vector<geometry_msgs::Pose>& path) {
  ...
  edge_marker.color.r = 0.0;
  edge_marker.color.g = 230.0 / 255.0;
  edge_marker.color.b = 255.0 / 255.0;
  edge_marker.color.a = 1.0;
  edge_marker.lifetime = ros::Duration(shortest_paths_lifetime);
  edge_marker.frame_locked = false;
  ...
  vertex_marker.scale.z = 0.15;
  vertex_marker.color.r = 255.0 / 255.0;
  vertex_marker.color.g = 165.0 / 255.0;
  vertex_marker.color.b = 0.0;
  vertex_marker.color.a = 1.0;
  vertex_marker.lifetime = ros::Duration(shortest_paths_lifetime);
  vertex_marker.frame_locked = false;
...
void MissionVisuals::visualizePath(
    const std::shared_ptr<CognitiveGraph> graph_manager,
  ...
  vertex_marker.color.r = 255.0 / 255.0;
  vertex_marker.color.g = 170.0 / 255.0;
  vertex_marker.color.b = 0.0;
  vertex_marker.color.a = 1.0;
  vertex_marker.lifetime = ros::Duration(shortest_paths_lifetime);
  vertex_marker.frame_locked = false;
...
void MissionVisuals::visualizeBestPaths(
    const std::shared_ptr<CognitiveGraph> graph_manager,
    const ShortestPathsReport& graph_rep, int n, int best_vertex_id) {
  if (selected_paths_pub_.getNumSubscribers() < 1) return;
  ...
  edge_marker.color.r = 255.0 / 255.0;
  edge_marker.color.g = 0.0;
  edge_marker.color.b = 220.0 / 255.0;
  edge_marker.color.a = 1.0;
  edge_marker.lifetime = ros::Duration(shortest_paths_lifetime);
  edge_marker.frame_locked = false;
  ...
  vertex_marker.color.r = 80.0 / 255.0;
  vertex_marker.color.g = 170.0 / 255.0;
  vertex_marker.color.b = 255.0 / 255.0;
  vertex_marker.color.a = 1.0;
  vertex_marker.lifetime = ros::Duration(shortest_paths_lifetime);
  ...
  selected_paths_pub_.publish(marker_array);
}

void MissionVisuals::visualizeRefPath(
    const std::vector<geometry_msgs::Pose>& path) {
  if (local_reference_path_pub_.getNumSubscribers() < 1) return;
  ...
  edge_marker.scale.x = 0.1;
  edge_marker.color.r = 255.0 / 255.0;
  edge_marker.color.g = 105.0 / 255.0;
  edge_marker.color.b = 0.0;
  edge_marker.color.a = 1.0;
  edge_marker.lifetime = ros::Duration(shortest_paths_lifetime);
  edge_marker.frame_locked = false;
  ...
  vertex_marker.color.r = 0.0;
  vertex_marker.color.g = 210.0 / 255.0;
  vertex_marker.color.b = 255.0 / 255.0;
  vertex_marker.color.a = 1.0;
  vertex_marker.lifetime = ros::Duration(shortest_paths_lifetime);
  vertex_marker.frame_locked = false;
  ...
  local_reference_path_pub_.publish(marker_array);
}
...
void MissionVisuals::visualizeRefPath(
  ...
  edge_marker.color.r = (color == 0 ? 255.0 : (color == 2 ? 130.0 : 0.0)) / 255.0;
  edge_marker.color.g = (color == 0 ? 140.0 : (color == 1 ? 230.0 : 80.0)) / 255.0;
  edge_marker.color.b = (color == 1 ? 255.0 : (color == 2 ? 255.0 : 0.0)) / 255.0;
  edge_marker.color.a = 1.0;
  edge_marker.lifetime = ros::Duration(shortest_paths_lifetime);
  edge_marker.frame_locked = false;
...
void MissionVisuals::visualizeVolumetricGain(
  ...
  unknown_voxel_marker.color.r = 105.0 / 255.0;
  unknown_voxel_marker.color.g = 55.0 / 255.0;
  unknown_voxel_marker.color.b = 155.0 / 255.0;
  ...
  free_voxel_marker.color.g = 125.0 / 255.0;
  free_voxel_marker.color.b = 95.0 / 255.0;
  ...
  occupied_voxel_marker.color.r = 190.0 / 255.0;
  occupied_voxel_marker.color.g = 55.0 / 255.0;
  occupied_voxel_marker.color.b = 30.0 / 255.0;
...
void MissionVisuals::visualizeSampler(RandomSampler& random_sampler) {
  ...
  invalid_samples_marker.color.r = 0.0;
  invalid_samples_marker.color.g = 1.0;
  invalid_samples_marker.color.b = 0.0;
...
void MissionVisuals::visualizeRays(
...
void MissionVisuals::visualizeRobotStateHistory(
...
void MissionVisuals::visualizeGeofence(
...
void MissionVisuals::visualizePCL(const pcl::PointCloud<pcl::PointXYZ>* pcl) {
  if (pcl_pub_.getNumSubscribers() < 1) return;

  ROS_WARN_COND(global_verbosity >= Verbosity::WARN, "[SubtPlanner Vis]: pcl size: %d", (int)(pcl->points.size()));
  if (pcl->points.size() <= 0) {
    ROS_WARN_COND(global_verbosity >= Verbosity::WARN, "[SubtPlanner Vis]: Empty PCL");
    return;
  }
...
void MissionVisuals::visualizeHyperplanes(
  ...
  for (int i = 0; i < hyperplane_list.size(); ++i) {
    ...
    marker_plane.scale.x = 0.1;
    marker_plane.scale.y = 5.0;
    marker_plane.scale.z = 5.0;
...
void MissionVisuals::visualizeModPath(
    const std::vector<geometry_msgs::Pose>& path) {
...
void MissionVisuals::visualizeBlindModPath(
    const std::vector<geometry_msgs::Pose>& path) {
  ...
  edge_marker.type = visualization_msgs::Marker::LINE_LIST;
  edge_marker.scale.x = 0.25;
  edge_marker.color.r = 90.0 / 255.0;
  edge_marker.color.g = 110.0 / 255.0;
  edge_marker.color.b = 255.0 / 255.0;
  edge_marker.color.a = 1.0;
  edge_marker.lifetime = ros::Duration(shortest_paths_lifetime);
  edge_marker.frame_locked = false;
  ...
  vertex_marker.scale.z = 0.25;
  vertex_marker.color.r = 255.0 / 255.0;
  vertex_marker.color.g = 150.0 / 255.0;
  vertex_marker.color.b = 0.0;
  vertex_marker.color.a = 1.0;
  vertex_marker.lifetime = ros::Duration(shortest_paths_lifetime);
  vertex_marker.frame_locked = false;
...
void MissionVisuals::visualizeViewpoints(std::vector<StateVec>& viewpoints)
{
  ...
  for (const auto& viewpoint : viewpoints) {
    ...
    tf::Quaternion quat;
    quat.setRPY(0, viewpoint[4], viewpoint[3]);
